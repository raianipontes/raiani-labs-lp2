...
public class Jogo {
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogadaJ1.getTipo() == TipoJogada.PASSA && jogadaJ2.getTipo() == TipoJogada.PASSA) {
			this.finalizado = true;
			this.vencedor = this.verificaDesempate();
		}

	}
	
	/**
	 * Verifica se há desempate através da quantidade de peças ou a soma dos valores das peças na mão.
	 * @return Nome do vencedor.
	 */
	public String verificaDesempate() {
		...
		if (this.jogador1.somaValoresPecaMao() < this.jogador2.somaValoresPecaMao()) {
			this.pontuacao = 1;
			return jogador1.getNome();
		}
		if (this.jogador1.somaValoresPecaMao() > this.jogador2.somaValoresPecaMao()) {
...
//		System.out.println("==> final: " + (venceu == null ? "EMPATE" : venceu + " VENCEU") + "\n");
	...
	/**
	 * Calcula a pontuação do jogador de acordo com a peça que ele bateu.
	 * Normal ou pós trancamento = 1
	 * Carroça = 2
	 * Lá e Ló = 3
	 * Cruzada = 6
	...
	private int calculaPontuacao(Jogador jogador) {
		...
		if (ultimaPeca.getNumDireito() == ultimaPeca.getNumEsquerdo()) {
			return 2;	
		}
		return 1;
